...
in disassembling this elf we can see that we have a string-format vulnerability and also the possibility to make a buffer overflow. And we can also see that we have a function that call ```system('/bin/sh')```. In order to win this challenge we need to redirect the flow of this binary program to this function.
...
I have written this little script to explore the stack try to find the exact position where the canary is.
...
with this dump of the stack (be careful, this is not the stack of one single execution, but every leak in an offset is an individual execution) the canary is at the offset 15. The stack value can be dumpt by sending the string: ```%15$p``` to the server.
...
Now we have a shell that allow us to get the flag!
